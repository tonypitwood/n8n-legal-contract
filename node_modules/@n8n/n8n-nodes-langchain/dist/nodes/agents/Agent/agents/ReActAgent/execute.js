...
__export(execute_exports, {
  reActAgentAgentExecute: () => reActAgentAgentExecute
});
...
async function reActAgentAgentExecute(nodeVersion) {
  ...
  await (0, import_utils.checkForStructuredTools)(tools, this.getNode(), "ReAct Agent");
  ...
  if ((0, import_helpers.isChatInstance)(model)) {
    agent = import_agents.ChatAgent.fromLLMAndTools(model, tools, {
      prefix: options.prefix,
      suffix: options.suffixChat,
      humanMessageTemplate: options.humanMessageTemplate
    });
  } else {
    agent = import_agents.ZeroShotAgent.fromLLMAndTools(model, tools, {
      prefix: options.prefix,
